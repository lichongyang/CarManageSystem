...
<div class="crumbs">
    ...
    <div id="inner-bd">
        <div class="kv-group-outer">
            <div class="kv-group clearfix">
                ...
                <div class="kv-item">
                    <div class="item-lt">条件：</div>
                    <div class="item-rt">
                        <select id="car_condition">
                        	<option value="car_type">车型</option>
                            <option value="car_class">系列</option>
                        </select>
                    </div>
                </div>
                <div class="kv-item">
    ...
    <script type="text/javascript">
    	...
    	$("#find").live('click', function(){
    		...
    		if (car_condition == "" || analyze_condition == "" || start_time == "" || end_time == ""){
    			alert("请填入相应信息");
    		}else{
    			var post_json = {
    					"s_name":s_name,
    			...
    			$.ajax({
    				url:"ClientServlet?methods=CarFashionAnalyze",
    				type:"post",
    				dataType:"json",
    				...
    				success:function(data){
    					...
    					var thead_html = "";
    					var tbody_html = "";
    					if (car_condition== "car_type" && analyze_condition == "family"){
    						thead_html += "<tr><td>类别</td><td>1</td><td>2</td><td>3</td><td>大于等于4</td>";
    						var mid_json = new Array();
    						$.each(data, function(index, result){
        						mid_json[result.car_type] = {
        								"1":0,
        								"2":0,
        								"3":0,
        								"大于等于4":0
    						...
    						$.each(data, function(index, result){
        						var calculate_num = mid_json[result.car_type][result.family_level] + result.statistics_num;
        						mid_json[result.car_type][result.family_level] = calculate_num;
        					});
    						...
    						for (var key in mid_json){
    							if (key == "remove")
    								continue;
    							$.each(data, function(index, result){
            						mid_json[key][result.car_type] = 0;
                				});
    						}
    						$.each(data, function(index, result){
        						var calculate_num = mid_json[result.career][result.car_type] + result.statistics_num;
        						mid_json[result.career][result.car_type] = calculate_num;
        					});
    						for (var key in mid_json){
    							if (key == "remove")
    								continue;
    					...
    					}else if (car_condition== "car_type" && analyze_condition == "age"){
    						...
    						$.each(data, function(index, result){
        						var calculate_num = mid_json[result.car_type][result.age_level] + result.statistics_num;
        						mid_json[result.car_type][result.age_level] = calculate_num;
        					});
    						for (var key in mid_json){
        						if (key == "remove"){
        							continue;
    					...
    					}else if (car_condition== "car_class" && analyze_condition == "family"){
    						thead_html += "<tr><td>类别</td><td>1</td><td>2</td><td>3</td><td>大于等于4</td>";
    						var mid_json = new Array();
    						$.each(data, function(index, result){
    						...
    						$.each(data, function(index, result){
        						var calculate_num = mid_json[result.car_class][result.family_level] + result.statistics_num;
        						mid_json[result.car_class][result.family_level] = calculate_num;
        					});
    						for (var key in mid_json){
        						if (key == "remove"){
        							continue;
    					...
    					}else if (car_condition== "car_class" && analyze_condition == "career"){
    						thead_html += "<tr><td>类别</td>"
    						var mid_json = new Array();
    						var type_mid_array = new Array();  //用于车辆类别数组去重，即先存至字典再取出
    						var type_array = new Array();      //统计查询到的车辆类别数组
    						$.each(data, function(index, result){
        						mid_json[result.career] = new Array();
        						type_mid_array[result.car_class] = 0;
            				});
    						for (var key in type_mid_array){
    							if (key == "remove")
    								continue;
    						...
    						for (var key in mid_json){
    							if (key == "remove")
    								continue;
    							$.each(data, function(index, result){
            						mid_json[key][result.car_class] = 0;
                				});
    						}
    						$.each(data, function(index, result){
        						var calculate_num = mid_json[result.career][result.car_class] + result.statistics_num;
        						mid_json[result.career][result.car_class] = calculate_num;
        					});
    						for (var key in mid_json){
    							if (key == "remove")
    								continue;
    					...
    					}else if (car_condition== "car_class" && analyze_condition == "age"){
    						thead_html += "<tr><td>类别</td><td>小于20</td><td>20至30</td><td>30至40</td><td>40至50</td><td>50至60</td><td>大于60</td></tr>";
    						var mid_json = new Array();
    						$.each(data, function(index, result){
        						mid_json[result.car_class] = {
    						...
    						$.each(data, function(index, result){
        						var calculate_num = mid_json[result.car_class][result.age_level] + result.statistics_num;
        						mid_json[result.car_class][result.age_level] = calculate_num;
        					});
    						for (var key in mid_json){
        						if (key == "remove"){
        							continue;
